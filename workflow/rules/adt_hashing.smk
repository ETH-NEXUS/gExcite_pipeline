...
rule CITE_seq_Count:
    ...
    output:
        run_report=report(
            "results/pooled_samples/citeseq_count/{sample_set}.run_report.yaml",
            caption="workflow/report/citeseq.rst",
            category="preprocessing QC",
        ),
    params:
        R1=lambda wildcards: ",".join(
            list_fastqs(
                config["inputOutput"]["input_fastqs_adt"]
                + "{sample_set}/".format(sample_set=wildcards.sample_set),
                "R1",
            )
        ),
        R2=lambda wildcards: ",".join(
            list_fastqs(
                config["inputOutput"]["input_fastqs_adt"]
                + "{sample_set}/".format(sample_set=wildcards.sample_set),
                "R2",
    ...
    shell:
        ...
        "-T {threads} "
        "-t {input.tags} ; "
        "ln -frs {params.outfile} {output.run_report} &> {log}"
...
rule gzip_files_hashingInput:
    ...
    shell:
        "mkdir -p {params.root_out} ; "
        "gzip -c '{input.features_file_tmp}' > '{output.features_file}' ; "
        "gzip -c '{input.matrix_file_tmp}' > '{output.matrix_file}' ; "
        "gzip -c '{input.barcodes_file_tmp}' > '{output.barcodes_file}' "
...
rule Rscript_analyseHashing:
    ...
    params:
        ...
        normalisation_downstream=config["tools"]["analyse_hashing"][
            "normalisation_downstream"
        ],
...
rule Rscript_demultiplex_count_matrix:
    ...
    shell:
        "Rscript {params.custom_script} "
        "--sampleMap {params.samplemapFile} "
        "--sample {wildcards.sample} "
        "--sampleSet {wildcards.sample_set} "
        "&> {log} "
