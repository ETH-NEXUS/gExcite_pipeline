...
novaFile <- args[1]
## assumes second parameter = root output folder, also for saving statistic files
## (cell calling comparison, summary tables, etc.)
outTag_nova <- args[2]
...
cat("\n")
tab_cells_nova <- data.frame(
  Samples = sampleNames_nova,
...
## Which molecules have no.samples>1? (ie. swapping)
## As each distinct tuple should only correspond to one sample (hence, no.reads>0 should only occur for one sample)
...
tab_nova <- data.frame(
  Reads = format(totalReads, big.mark = ","),
...
tab_swap_samples_num <- data.frame(
  Samples = sampleNames_nova,
  Reads = rep(NA, length(sampleNames_nova)),
  Molecules = rep(NA, length(sampleNames_nova)),
  Swapped_molecules = rep(NA, length(sampleNames_nova)),
...
if (sum(samples_5pr) < 1 | sum(samples_3pr) < 1) {
  ...
  tab_swap_libs <- data.frame(
    Libraries = library_types,
  ...
  tab_swap_split <- data.frame(
    Libraries = swap_categs,
  ...
  ## Report in table
  tab_swap_split[1, 2:4] <- c(
    format(total_swapped, big.mark = ","), format(molecules_3pr, big.mark = ","),
    paste0(format(swapped_fraction_3pr, digits = 3), " %")
  )
  tab_swap_split[2, 2:4] <- c(
    format(total_swapped, big.mark = ","), format(molecules_5pr, big.mark = ","),
    paste0(format(swapped_fraction_5pr, digits = 3), " %")
  )
  tab_swap_split[3, 2:4] <- c(
    format(total_swapped, big.mark = ","), format(mixed_molecules, big.mark = ","),
    paste0(format(swapped_fraction_mixed, digits = 3), " %")
  )
  ...
  cat("\n\nOutput files and directories:\n")
  cat(paste(" ", c(
    novaFile, summary_all_nova, summary_samples_nova, summary_libs_nova,
    summary_events_nova, outFiles_nova
  ), collapse = "\n"), "\n\n")
}
