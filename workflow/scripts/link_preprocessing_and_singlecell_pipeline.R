...
filter_cellRanger <- function(barcodeList, inputFolder, outputFolder, hashedSampleSet, sampleName) {
    ...
    mat = readMM(file = paste(inputFolder,paste(hashedSampleSet,"matrix.mtx",sep="."),sep="/"))
    print(dim(mat))
    print(str(mat))
    ...
    gzip(outbarcodes, paste(zippFileDir,"barcodes.tsv.gz",sep="/"), overwrite=TRUE, remove=FALSE)
    gzip(outmatrix, paste(zippFileDir,"matrix.mtx.gz",sep="/"), overwrite=TRUE, remove=FALSE)
    gzip(outfeatures, paste(zippFileDir,"features.tsv.gz",sep="/"), overwrite=TRUE, remove=FALSE)
}
